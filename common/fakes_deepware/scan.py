...
from common.fakes_deepware.mtcnn_mod import MTCNN
...
def confident(p):
    return np.mean(np.abs(p - 0.5) * 2) >= 0.7


def label_spread(x):
    return x - np.log10(x) if x >= 0.8 else x
...
def process(file):
    try:
        ...
        return np.clip(score, 0.01, 0.99).item()
...
def create_scanner(device="cpu"):
    models_dir = Path(root_dir, "weights")
    cfg_file = Path(root_dir, "deepfake_scanner/config.json")
